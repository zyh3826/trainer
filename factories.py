...
def scheduler_factory(config: CfgNode, optimizer: optim.Optimizer):
    ...
    if config.scheduler.type == 'StepLR':
        ...
        return optim.lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max=config.scheduler.get('T_max', 1000),
        ...
        return optim.lr_scheduler.CosineAnnealingWarmRestarts(
            optimizer,
            T_0=config.scheduler.get('T_0', 100),
            T_mult=config.scheduler.get('T_mult', 100),
            eta_min=config.scheduler.get('eta_min', 1e-7),
            last_epoch=config.scheduler.get('last_epoch', -1)
            )
    else:
...
def scheduler_with_warmup_factory(
                                ...
                                lr_end: float = 1e-7,
                                power: float = 1.0) -> optim.lr_scheduler.LambdaLR:
    if config.scheduler.type == 'constant':
        return get_constant_schedule_with_warmup(optimizer, num_warmup_steps=num_warmup_steps, last_epoch=last_epoch)
